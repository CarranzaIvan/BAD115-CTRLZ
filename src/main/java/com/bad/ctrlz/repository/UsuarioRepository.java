...
public interface UsuarioRepository extends JpaRepository<Usuario, Integer> {
  ...
  // Cuenta usuarios que solicitan ser desbloqueados
  @Query("""
          SELECT COUNT(u)
          FROM Usuario u
          WHERE u.accountLocked = true
            AND u.enabled = false AND u.accountNoExpired = false
      """)
  int contarSolicitudDesbloqueo();

  // Listado de bloqueados
  @Query("""
          SELECT u
          FROM Usuario u
          WHERE u.accountLocked = true
  ...
  @Query("""
          ...
          FROM Usuario u
          WHERE u.accountLocked = true
            AND u.accountNoExpired = false
      """)
  List<Usuario> findSolicitud();

  // Si también necesitas contar las solicitudes (método adicional)
  @Query("""
          SELECT COUNT(u)
          FROM Usuario u
          WHERE u.accountLocked = true
            AND u.enabled = false AND u.accountNoExpired = false
      """)
  int contarSolicitudesDesbloqueo();
}